...
file_protected int file_os2_apptype(struct magic_set *ms, const char *fn,
	...
	(void)sprintf(path, "%s%s%s%s", drive, (*dir == '\0') ? "./" : dir,
	              fname, (*ext == '\0') ? "." : ext);

	if (fn == NULL) {
		if ((fp = fopen(path, "wb")) == NULL) {
			file_error(ms, errno, "cannot open tmp file `%s'",
			           path);
			return -1;
		}
	...
	switch (type & (FAPPTYP_NOTWINDOWCOMPAT | FAPPTYP_WINDOWCOMPAT |
	                FAPPTYP_WINDOWAPI)) {
